"""app.py — NutriBot v5.0 — Complete Multi-User Implementation."""
...
def login_gate():
    ...
    if login_mode == "url":
        ...
        if raw:
            ...
            return
        # No param — fall through to form so user isn't stranded
        login_mode = "form"
...
# ═══════════════════════════════════════════════════════════════════════════
# PER-USER SERVICE FACTORY  (replaces the old @st.cache_resource version)
# ═══════════════════════════════════════════════════════════════════════════
...
def get_user_services(user_id: str) -> dict:
    ...
    # This call creates per-user GroceryDB, ProfileDB, FeedbackDB
    svc = _build(user_id)
    svc["client"]    = client
    svc["user_id"]   = user_id

    # Recipe knowledge base is read-only/global — safe to share
    dataset = load_recipe_dataset()
    svc["recipe_kb"] = build_recipe_knowledge_base(dataset)

    # Stateless agents — safe to create fresh per session
...
def init_session():
    """Set all session-state defaults (does NOT touch user_id — login_gate owns that)."""
    defaults = {
        "chat_history":   [],
        "turn_count":     0,
        "chip_query":     None,
...
# ═══════════════════════════════════════════════════════════════════════════
# CARD RENDERERS  (unchanged from original)
# ═══════════════════════════════════════════════════════════════════════════
...
def render_sidebar(services: dict):
    ...
    with st.sidebar:
        ...
        if st.button("🚪 Switch User", use_container_width=True):
            # Clear only the user-specific keys; keep Streamlit plumbing
...
| 🔥 Calories | {profile.get("calorie_goal", "—")} kcal/meal |
                ...
                if st.button("🗑️ Reset Profile", use_container_width=True):
                    ...
                    # Evict cached services so they rebuild with fresh profile
...
# ═══════════════════════════════════════════════════════════════════════════
# PIPELINE RUNNER  — injects user_id so agents use the right DB
# ═══════════════════════════════════════════════════════════════════════════
...
def run_pipeline(prompt: str, services: dict) -> dict:
    ...
    state = build_initial_state(
        user_query           = prompt,
        user_id              = user_id,          # ← KEY: pass user_id into state
...
def main():
    # 1. Authentication — must happen before anything else
    login_gate()
            ...
            if steps:
                ...
                with col2:
                    if st.button("✕ Exit", use_container_width=True):
                        st.session_state["cooking_mode"] = False; st.rerun()
                with col3:
                    if idx < total - 1:
    ...
    with col1:
        if st.button("⌨️ Text",      use_container_width=True, type="primary" if st.session_state["input_mode"] == "text"      else "secondary"): st.session_state["input_mode"] = "text";      st.rerun()
    with col2:
        if st.button("🎙️ Voice",     use_container_width=True, type="primary" if st.session_state["input_mode"] == "voice"     else "secondary"): st.session_state["input_mode"] = "voice";     st.rerun()
    with col3:
        if st.button("📸 Photo",     use_container_width=True, type="primary" if st.session_state["input_mode"] == "image"     else "secondary"): st.session_state["input_mode"] = "image";     st.rerun()
    with col4:
        if st.button("🧾 Bill",      use_container_width=True, type="primary" if st.session_state["input_mode"] == "bill"      else "secondary"): st.session_state["input_mode"] = "bill";      st.rerun()
    with col5:
        if st.button("🎤 Assistant", use_container_width=True, type="primary" if st.session_state["input_mode"] == "assistant" else "secondary"): st.session_state["input_mode"] = "assistant"; st.rerun()
